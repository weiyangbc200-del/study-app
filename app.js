 ...
 * ✅ 起床/睡覺：可手動輸入 + 重置（防誤按覆蓋）+ 匯出 JSON 記錄所有操作
 ************************/
...
const K = {
  ...
  // NEW: wake/sleep audit log
  timeMarks: "timeMarks", // [{id, day, kind, value, source, at, prev}]
};
...
const data = {
  ...
  // NEW
  timeMarks: loadJSON(K.timeMarks, DEFAULTS.timeMarks),
...
function persistAll(){
  ...
  // NEW
  saveJSON(K.timeMarks, data.timeMarks);
}
...
/** ========= Wake/Sleep manual reset + audit log ========= */
function todayKey(){ return dateKey(new Date()); }

function normalizeTimeMarks(){
  data.timeMarks = (data.timeMarks || []).map(x=>({
    id: x.id || crypto.randomUUID(),
    day: x.day || todayKey(),
    kind: (x.kind === "sleep" ? "sleep" : "wake"),
    value: (x.value ?? ""),
    source: x.source || "unknown", // button | manual | reset
...
function logTimeMark(kind, value, source){
  if(!data.timeMarks) data.timeMarks = [];
  const prev = (kind === "wake") ? (data.wakeTime ?? "") : (data.sleepTime ?? "");
  data.timeMarks.unshift({
...
function setWakeTimeValue(hhmm, source){
  data.wakeTime = hhmm ?? "";
  logTimeMark("wake", data.wakeTime, source);
  persistAll();
}
function setSleepTimeValue(hhmm, source){
  data.sleepTime = hhmm ?? "";
  logTimeMark("sleep", data.sleepTime, source);
  persistAll();
}

function promptTimeInput(label, fallback){
  const v = prompt(`${label}（HH:MM）`, fallback || "");
  if(v === null) return null; // cancel
  const s = String(v).trim();
  if(!s) return ""; // allow clear
  if(!/^\d{2}:\d{2}$/.test(s)) { alert("格式錯誤，請用 HH:MM（例 07:30）"); return null; }
  const [hh, mm] = s.split(":").map(Number);
  if(hh < 0 || hh > 23 || mm < 0 || mm > 59){ alert("時間範圍錯誤"); return null; }
...
function renderCalendar(){
  ...
  return `
    ...
    <div class="card">
      ...
      <!-- Wake / Sleep with manual & reset -->
      <div style="margin-top:12px; display:flex; flex-direction:column; gap:10px;">
        <div class="row-between" style="gap:10px;">
          <button class="btn primary" data-action="markWake" style="flex:1; min-width:0;">
            起床（按下記錄現在）：<span class="mono">${escapeHtml(data.wakeTime || "--:--")}</span>
          </button>
          <div class="row" style="gap:8px; flex:0 0 auto;">
            <button class="btn" data-action="setWakeManual">手動</button>
            <button class="btn danger" data-action="clearWake">重置</button>
          </div>
        </div>

        <div class="row-between" style="gap:10px;">
          <button class="btn" data-action="markSleep" style="flex:1; min-width:0;">
            睡覺（按下記錄現在）：<span class="mono">${escapeHtml(data.sleepTime || "--:--")}</span>
          </button>
          <div class="row" style="gap:8px; flex:0 0 auto;">
            <button class="btn" data-action="setSleepManual">手動</button>
            <button class="btn danger" data-action="clearSleep">重置</button>
          </div>
        </div>

        <div class="small" style="opacity:0.9;">
          ✅ 手動可改回正確時間；重置會清空成 --:--。所有操作都會記錄進匯出 JSON 的 timeMarks。
        </div>
      </div>
...
function renderDashboard(){
  ...
  return `
    ...
    <div class="card">
      ...
      <div class="row-between" style="margin-top:8px;"><div>睡覺時間</div><div class="mono"><b>${escapeHtml(data.sleepTime || "--:--")}</b></div></div>
...
function bindDynamicHandlers(){
  ...
  // 起床/睡覺：按下即記錄當下時間（有值時先 confirm 避免誤按覆蓋）
  elContent.querySelectorAll("[data-action='markWake']").forEach(btn=>{
    btn.onclick = ()=>{
      if((data.wakeTime || "").trim()){
        if(!confirm(`已經有起床時間：${data.wakeTime}\n仍要用「現在時間」覆蓋嗎？`)) return;
      }
      setWakeTimeValue(fmtTime(new Date()), "button");
      render();
    };
  });
  ...
  elContent.querySelectorAll("[data-action='markSleep']").forEach(btn=>{
    btn.onclick = ()=>{
      if((data.sleepTime || "").trim()){
        if(!confirm(`已經有睡覺時間：${data.sleepTime}\n仍要用「現在時間」覆蓋嗎？`)) return;
      }
      setSleepTimeValue(fmtTime(new Date()), "button");
      render();
    };
  });

  // 手動輸入 / 重置
  elContent.querySelectorAll("[data-action='setWakeManual']").forEach(btn=>{
    btn.onclick = ()=>{
      const v = promptTimeInput("手動輸入起床時間", data.wakeTime || DEFAULTS.wakeTime || "07:00");
      if(v === null) return;
      setWakeTimeValue(v, "manual");
      render();
    };
  });
  elContent.querySelectorAll("[data-action='setSleepManual']").forEach(btn=>{
    btn.onclick = ()=>{
      const v = promptTimeInput("手動輸入睡覺時間", data.sleepTime || DEFAULTS.sleepTime || "23:30");
      if(v === null) return;
      setSleepTimeValue(v, "manual");
      render();
    };
  });
  elContent.querySelectorAll("[data-action='clearWake']").forEach(btn=>{
    btn.onclick = ()=>{
      if(!confirm("確定要重置起床時間？（會清空成 --:--）")) return;
      setWakeTimeValue("", "reset");
      render();
    };
  });
  elContent.querySelectorAll("[data-action='clearSleep']").forEach(btn=>{
    btn.onclick = ()=>{
      if(!confirm("確定要重置睡覺時間？（會清空成 --:--）")) return;
      setSleepTimeValue("", "reset");
      render();
    };
  });
...
function importDataObj(obj){
  ...
  data.timeMarks = Array.isArray(d.timeMarks) ? d.timeMarks : []; // NEW
  normalizeTimeMarks();
